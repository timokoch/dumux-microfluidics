...
tilting_speeds = [0.5, 1.0]
#volume = np.linspace(100, 800, endpoint=True, num=10)
transmissibility_factor = [0.1, 0.5, 1.0]

#reservoir = "small"
reservoir = "big"

X, Y = np.meshgrid(tilting_speeds, transmissibility_factor)

def max_wss(speed, tilt):
    def generate():
        for s, t in zip(speed, tilt):
            if reservoir == "big":
                file_name = f"lowre_s{s}_t{t}_p0_300ul_d500_big.txt"
            else:
                file_name = f"lowre_s{s}_t{t}_p0_200ul_d500_small.txt"
...
fig.tight_layout()
plt.show()
fig.savefig("lowre_small_wireframe.pdf", dpi=600)
plt.show()
