...
int main(int argc, char** argv)
{
    ...
    if (getParam<bool>("LinearSolver.UseDirectSolver", false))
    {
        ...
        numSolverIterations = linearSolver->result().iterations;
    }
    else
    {
        using Matrix = typename Assembler::JacobianMatrix;
        using Vector = typename Assembler::ResidualType;
        using LinearSolver = Dumux::IncompressibleStokesSolver<Matrix, Vector, MomentumGridGeometry, MassGridGeometry>;
        ...
        for (const auto& element : elements(leafGridView))
        {
            ...
            if constexpr (MomentumGridGeometry::discMethod == DiscretizationMethods::fcdiamond)
            {
    ...
    std::cout << "Solver iterations: " << numSolverIterations << "\n";
